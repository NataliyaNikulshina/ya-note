...
def test_user_can_create_note(author_client, author, form_data):
    """
    залогиненного пользователя должно переадресовать на страницу
    успешного создания заметки;
    нужно проверить, что заметка добавлена в БД и что значения
...
def test_anonymous_user_cant_create_note(client, form_data):
    """
    с анонимным пользователем всё иначе:
    после POST-запроса его должно переадресовать на страницу логина,
    а в базе не должно появиться новых заметок.
    """
    url = reverse('notes:add')
...
def test_not_unique_slug(author_client, note, form_data):
    """Тест на возможность создать две заметки с одинаковым slug."""
    url = reverse('notes:add')
...
def test_empty_slug(author_client, form_data):
    """
    Тест: если при создании заметки оставить поле slug пустым — 
    то содержимое этого поля будет сформировано автоматически, 
    из содержимого поля title.
    """
    url = reverse('notes:add')
...
def test_other_user_cant_edit_note(not_author_client, form_data, note):
    """Тестирование редактирования чужой заметки"""
    url = reverse('notes:edit', args=(note.slug,))
...
def test_other_user_cant_delete_note(not_author_client, slug_for_args):
    """Тестирование удаления чужой заметки"""
    url = reverse('notes:delete', args=slug_for_args)
